/////////////// OTA_BME280_draft2 ////////////////////
//
//  Remember to use 'reset' button after serial port download; otherwise OTA uploads may not work.
//
...
//
// 2019-06-06 tested, working
// 2019-06-25 tested, working after addition of client.setInsecure();
//
//////////////////////////////////////////////////////
...
const char versionstring[] = "20190625.0210.1";
...
#ifdef PHANT01
/////////////// Phant ////////////////////////
char phantHost [] = MYPHANTHOST;
char phantWebPage [] = MYPHANTWEBPAGE;
char phantPubKey [] = MYPHANTPUBKEY;
char phantPrivKey [] = MYPHANTPRIVKEY;
char phantCertFingerprint [] = PHANTSHA1FINGERPRINT;
...
void setup() {
  ...
  // Identification
  Serial.println();
  Serial.print("file: ");                   Serial.println(vfname);
  Serial.print("timestamp (local time): "); Serial.println(vtimestamp);
  Serial.println();
...
void handle_OnConnect() {
  ...
  server.send(200, "text/html", SendHTML(temperatureC,temperatureF,humidity,pressure,altitude)); 
}
...
String SendHTML(float temperatureC, float temperaturef, float humidity,float pressure,float altitude){
...
void handlePhantReport() {
  ...
  Serial.print("sending data to ");  Serial.println(phantHost);
  ...
  float barom = bme.readPressure() / 100.0F;
   ...
   if (client.verify(phantCertFingerprint, phantHost)) {
     Serial.println("certificate matches"); // With BearSSL set to Insecure mode, it will always report that it matches
   } else {
     Serial.println("certificate doesn't match");
     // return;
   }

  String ReqData = "barom=";         ReqData += barom;
        ReqData += "&humidity=";     ReqData += humidity;
        ReqData += "&tempf=";        ReqData += tempf;
        ReqData += "\r\n";
  Serial.println("ReqData= " + ReqData);

  String WebReq = "POST ";         WebReq += phantWebPage;     WebReq += " HTTP/1.1\r\n";
  WebReq += "Host: ";              WebReq += phantHost;        WebReq += "\r\n";
